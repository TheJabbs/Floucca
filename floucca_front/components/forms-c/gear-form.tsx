...
const GearInfo: React.FC<GearInfoProps> = ({ onChange }) => {
  ...
  const [currentSelection, setCurrentSelection] = useState<GearEntry>({ gearId: 0, months: [] });
  ...
  const addGear = () => {
    if (currentSelection.gearId === 0 || currentSelection.months.length === 0) return;

    setGearEntries((prev) => [...prev, currentSelection]);
    setCurrentSelection({ gearId: 0, months: [] });
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-5">
          <select
            value={currentSelection.gearId}
            onChange={(e) => setCurrentSelection((prev) => ({ ...prev, gearId: Number(e.target.value) }))}
          ...
          {/* Add Gear Button */}
          <AddButton onClick={addGear} disabled={currentSelection.gearId === 0 || currentSelection.months.length === 0} />
        </div>
