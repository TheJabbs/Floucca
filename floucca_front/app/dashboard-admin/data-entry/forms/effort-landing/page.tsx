...
function EffortAndLandingPage() {
  ...
  const { gears, species, ports, periods, isLoading, error: dataError, refetch } = useFormsData();
  const { user } = useAuth();
  ...
  // Update port and period when selected from dropdowns
  useEffect(() => {
    ...
    try {
      ...
      const apiPayload: LandingFormDTO = {
        form: {
          ...
          // Include selected period if provided
          ...(formData.period && { period_date: new Date(formData.period) }),
        },
        boat_details: {
