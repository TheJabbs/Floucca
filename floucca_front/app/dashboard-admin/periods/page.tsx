...
import { ChevronDown, ChevronUp, Calendar, Edit, Save, X, RefreshCw, Filter, AlertTriangle} from "lucide-react";
import {
...
const PeriodManagement = () => {
    ...
    const [editingActiveDays, setEditingActiveDays] = useState<Record<number, string>>({});
    ...
    const onSubmitStatusUpdate = async (data: { period_status: string }) => {
        ...
        try {
            ...
            // update local state
            setPeriods(
                periods.map((p) => p.period.period_date === editingStatus ? {
                        ...p,
                        period: {
                            ...p.period,
    ...
    const saveActiveDayUpdate = async (activeId: number) => {
        try {
            ...
            const currentPeriod = periods.find((p) => p.activeDays.some((a) => a.active_id === activeId));
            const maxDays = currentPeriod ? getDaysInMonthByDate(currentPeriod.period.period_date) : 31;
            ...
            setPeriods(
                periods.map((p) => ({
                    ...p,
                    activeDays: p.activeDays.map((a) => a.active_id === activeId ? { ...a, active_days: newValue } : a),
                }))
            );
    ...
    const getFilteredActiveDays = (activeDays: ActiveDay[]) => {
        return activeDays.filter((day) => {
            const portMatch = selectedPort ? day.port_id === parseInt(selectedPort) : true;
            const gearMatch = selectedGear ? day.gear_code === parseInt(selectedGear) : true;
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div
                                        className={`px-3 py-1 rounded-full text-sm border ${
                                            STATUS_COLORS[period.period.period_status as keyof typeof STATUS_COLORS] || "bg-gray-100"
                                        }`}
                                    >
                                        {STATUS_LABELS[period.period.period_status as keyof typeof STATUS_LABELS] || period.period.period_status}
                                    </div>
                                </div>
